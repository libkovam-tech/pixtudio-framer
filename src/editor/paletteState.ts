...
function parsePaletteStateColorToRgb(color: string): RgbColor | null {
    const normalized = normalizeSwatchColor(color)
...
export function ensurePaletteSwatchVisibleInGrid<TPixel extends string | null>(
    input: {
        imagePixels: TPixel[][]
        sourcePixels: (string | null)[][]
        swatchId: string
    ...
    if (
        input.imagePixels.some((row) =>
            row.some((value) => value === input.swatchId)
        )
    ) {
        return input.imagePixels
    }

    const targetRgb = parsePaletteStateColorToRgb(input.swatchColor)
    if (!targetRgb) return input.imagePixels

    const candidates: Array<{ row: number; column: number; score: number }> = []
    let paintableCells = 0

    for (let row = 0; row < input.imagePixels.length; row++) {
        const imageRow = input.imagePixels[row] ?? []
        const sourceRow = input.sourcePixels[row] ?? []
        for (let column = 0; column < imageRow.length; column++) {
            if (imageRow[column] == null) continue

            const sourceColor = sourceRow[column] ?? null
            if (sourceColor == null) continue

            const sourceRgb = parsePaletteStateColorToRgb(sourceColor)
            ...
            candidates.push({
                row,
                column,
                score: rgbDistanceSq(sourceRgb, targetRgb),
            })
        }
    }

    if (candidates.length === 0 || paintableCells === 0) {
        return input.imagePixels
    }

    const minCellRatio = input.minCellRatio ?? 0.01
    const minCells = input.minCells ?? 1
    const maxCells = input.maxCells ?? 256
    const targetCells = Math.max(
        minCells,
        Math.ceil(paintableCells * minCellRatio)
    )
    const count = Math.min(maxCells, targetCells, candidates.length)
    const selected = candidates
        .sort(
            (a, b) =>
                a.score - b.score || a.row - b.row || a.column - b.column
        )
        .slice(0, count)

    const out = input.imagePixels.map((row) => row.slice())
    for (const item of selected) {
        out[item.row][item.column] = input.swatchId as TPixel
    }
